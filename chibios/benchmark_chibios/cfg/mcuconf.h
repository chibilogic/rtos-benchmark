...
/* ADR-014 strada A1: TIM2 is driven directly (not via the GPT
 * driver) by test_ctxsw_irq.c so we can use CC1 IRQ as the wakeup
 * source, with PA0 driven HIGH by the same compare event in PWM
 * mode 2. Keep the GPT use disabled to free the TIM2 IRQ vector
 * for our custom handler. */
 ...
 *
 * ADR-012: OSAL_ST_MODE is derived automatically from
 * CH_CFG_ST_TIMEDELTA inside osal.h:
 *   CH_CFG_ST_TIMEDELTA == 0 -> PERIODIC (Cortex SysTick, fair_perf)
 *   CH_CFG_ST_TIMEDELTA != 0 -> FREERUNNING (TIMx, realistic_tickless)
 * STM32_ST_USE_TIMER is read only in FREERUNNING; we set it to TIM5
 * (32-bit, free) for realistic_tickless. TIM2 is always reserved
 * for the T1 benchmark.
 */
